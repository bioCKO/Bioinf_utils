...
outlong.write('Gene\t5 prime intergenic length\tMotif count\tMotif positions(plus strand)\tMotif sequences(plus strand)\tMotif positions(minus strand)\tMotif sequences(minus strand)\n')
...
for prom in proms:
    ...
    if len(lsn) > 0 or len(lsp) > 0:
         outlong.write(prom + '\t' + str(len(proms[prom])) + '\t' + str(len(lsp)+len(lsn)) + '\t' + ','.join(map(str,lsp)) + '\t' + ','.join(motlisp) + '\t' + ','.join(map(str,lsn)) + '\t' + ','.join(motlisn)+ '\n')
