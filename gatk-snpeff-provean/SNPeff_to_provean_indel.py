...
infile = glob.glob('/Volumes/MP_HD/bys_snpeff_catchup/initial_snpeff_snp_vcfs/*.vcf')
...
#     outfile = open('/Volumes/MP_HD/bys_snpeff_catchup/' + strain + '_indel_provean_inp.txt','w')
...
#         if j[0] == '#':
#             continue
#         elif '|inframe_insertion|' in j or 'inframe_deletion|' in j:
#             col = j.strip().split('\t')
#             inf = col[7].split(';')
#             for t in inf:
#                 if t[:3] == 'ANN':
...
for i in infile:
    ...
    strain = last[:last.find('_')]
    strains.append(strain)
    outfile = open('/Volumes/MP_HD/bys_snpeff_catchup/all_genes_snpeff_snp_vcf/' + strain + '_snp_provean_inp.txt','w')
    inf = open(i,'r').readlines()
    frameshift_pos = {}
    frameshift_pc_left = {}
    for j in inf:
