...
ingene = open("/Volumes/MP_HD/repDNA_data/by")
...
def merge_counts_lists(flank5,flank3,countls):
    ...
    for i in range(0,21000-1):
        sumval = sum([eq_counts[j][i] for j in sorted(eq_counts.keys())])
...
title("50 control genes",fontsize=12)

#plt.show()
plt.savefig("/Volumes/MP_HD/repDNA_data/all_3_families/50_gene_control_sum_of_rep_counts.pdf",dpi=200)