...
for pos in ends5:
    ...
    for i in gene:
        if gene[i][0] == pos[0] and gene[i][1] < pos[2] and i in delgenes:
        ...
        elif gene[i][0] == pos[0] and gene[i][1] < pos[2] and i in helicases:
            orient = int(gene[i][3] + '1')
            if orient > 0:
                feature = SeqFeature(FeatureLocation(gene[i][1], gene[i][2]), strand=orient)
...
# gdd.write("/Volumes/MP_HD/repDNA_data/telomere_figures_frm_script/5_prime_"+sname+"kb_del_genes.pdf", "pdf")

gdd.write("/Volumes/MP_HD/repDNA_data/telomere_figures_frm_script/5_prime_"+sname+"kb_del_genes_"+strain+".pdf", "pdf")
...
for pos in ends3:
    ...
    for i in gene:
        ...
        gend = gene[i][2] - pos[1]
        if gene[i][0] == pos[0] and gend > 0 and i in delgenes:
            orient = int(gene[i][3] + '1')
            if orient > 0:
                ...
                feature = SeqFeature(FeatureLocation(gstart, gend), strand=orient)
                gd_feature_set.add_feature(feature, name=i, label=lab, sigil="ARROW", color="red", arrowshaft_height=athick, label_size = lsize, label_angle=150)
        elif gene[i][0] == pos[0] and gend > 0 and i in helicases:
            orient = int(gene[i][3] + '1')
            if orient > 0:
                feature = SeqFeature(FeatureLocation(gstart, gend), strand=orient)
                gd_feature_set.add_feature(feature, name=i, label=lab, sigil="ARROW", color="teal", arrowshaft_height=athick, label_size = lsize, label_angle=30)
    ...
    if pos[1] == 0:
        ...
        feature = SeqFeature(FeatureLocation(pos[2]-pos[1]-200, pos[2]), strand=0)
        gd_feature_set.add_feature(feature, name="Scaffold end", label=lab, sigil="BOX", color="red", arrowshaft_height=athick,  label_size = lsize, label_angle=30)
...
# gdd.write("/Volumes/MP_HD/repDNA_data/telomere_figures_frm_script/3_prime_"+sname+"kb_del_genes.pdf", "pdf")

gdd.write("/Volumes/MP_HD/repDNA_data/telomere_figures_frm_script/3_prime_"+sname+"kb_del_genes_"+strain+".pdf", "pdf")
