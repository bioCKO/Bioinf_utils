...
##delly
indels = ["/Volumes/MP_HD/CI_GENOME_SEQ/CI_delly/filt_precise_all_DEL.vcf"]
ininvs = ["/Volumes/MP_HD/CI_GENOME_SEQ/CI_delly/filt_precise_all_INV.vcf"]
indupe = ["/Volumes/MP_HD/CI_GENOME_SEQ/CI_delly/filt_precise_all_DUP.vcf"]
intran = ["/Volumes/MP_HD/CI_GENOME_SEQ/CI_delly/filt_precise_all_TRA.vcf"]
...
#    ax0.set_yticks([250,500,750,1000])
    ...
    ax1.plot(X,Y2, lw=1, antialiased = True, alpha=0.5, color = 'green', label = "Inversions",)
    Y3 = numpy.asarray(dupvalues[contig])
    ax1.plot(X,Y3, lw=1, antialiased = True, alpha=0.5, color = 'blue', label = "Duplication",)
    Y4 = numpy.asarray(travalues[contig])
    ax1.plot(X,Y4, lw=1, antialiased = True, alpha=0.5, color = 'orange', label = "Transpositions",)
    # ax1.set_ylim([0,120])
