...
for j in nlist:
    ...
    species = ['Pma','Tfl','Pfu','Tst','Pde','Pro','Pch','Pdi','ANID','Ani','Ate','Afl','Aor','Afu','Nfi','Acl','Ade','Hca','Pbr','CIM','Ure','Teq','Tto']

    # for i in range(1,len(sys.argv)-1):
    #     species.append(sys.argv[i].split('/')[-1][:-12])
    species.remove(j)

    names[j] = pm_dict

    outfile.write('InterPro ID\t' + j + '\t' + '\t'.join(species) + '\tpvalue\tcorrected P value\tup/down ratio\n')

    st = 0
    for i in ids:
        outfile.write(i + '\t' + str(pm_dict[i][0]) + '\t')
        for k in species:
            outfile.write(str(names[k][i][0]) + '\t')
